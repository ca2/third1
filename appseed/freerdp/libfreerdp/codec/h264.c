...
static int av_decode_video(AVCodecContext *avctx, AVFrame *frame, int *got_frame, AVPacket *pkt)
{
   int ret = 0;
                                 ...
                                 UINT32 SrcSize, UINT32 plane)
{
   ...
   packet.size = SrcSize;
   status = av_decode_video(sys->codecContext, sys->videoFrame, &gotFrame,
                            &packet);
