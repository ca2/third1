...
// TIFF Exif profile
WINBOOL tiff_read_exif_tags(TIFF *tif, TagLib::MDMODEL md_model, FIBITMAP *dib);
WINBOOL tiff_write_exif_tags(TIFF *tif, TagLib::MDMODEL md_model, FIBITMAP *dib);
...
static FIBITMAP* CreateImageType(WINBOOL header_only, FREE_IMAGE_TYPE fit, int width, int height, uint16 bitspersample, uint16 samplesperpixel);
...
static WINBOOL tiff_read_iptc_profile(TIFF *tiff, FIBITMAP *dib);
static WINBOOL tiff_read_xmp_profile(TIFF *tiff, FIBITMAP *dib);
static WINBOOL tiff_read_exif_profile(TIFF *tiff, FIBITMAP *dib);
static void ReadMetadata(TIFF *tiff, FIBITMAP *dib);

static WINBOOL tiff_write_iptc_profile(TIFF *tiff, FIBITMAP *dib);
static WINBOOL tiff_write_xmp_profile(TIFF *tiff, FIBITMAP *dib);
...
*/
static FIBITMAP*
CreateImageType(WINBOOL header_only, FREE_IMAGE_TYPE fit, int width, int height, uint16 bitspersample, uint16 samplesperpixel) {
	FIBITMAP *dib = NULL;
...
FindLoadMethod(TIFF *tif, FREE_IMAGE_TYPE image_type, int flags) {
	...
	WINBOOL bIsTiled = (TIFFIsTiled(tif) == 0) ? FALSE:TRUE;
...
Load(FreeImageIO *io, fi_handle handle, int page, int flags, void *data) {
	...
	const WINBOOL header_only = (flags & FIF_LOAD_NOPIXELS) == FIF_LOAD_NOPIXELS;
	...
	try {
		...
		const WINBOOL asCMYK = (flags & TIFF_CMYK) == TIFF_CMYK;
		...
		if(loadMethod == LoadAsRBGA) {
			...
			WINBOOL isCMYKA = (photometric == PHOTOMETRIC_SEPARATED) && (samplesperpixel > 4);
...
Save(FreeImageIO *io, FIBITMAP *dib, fi_handle handle, int page, int flags, void *data) {
	...
	const WINBOOL bHasThumbnail = (FreeImage_GetThumbnail(dib) != NULL);
